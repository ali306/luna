...
impl TerminationStrategy {
    fn execute(&self) {
        debug!(
            "Executing graceful termination: {} {:?}",
            self.graceful_cmd, self.graceful_args
        );
        let _ = Command::new(&self.graceful_cmd)
            .args(&self.graceful_args)
            .output();

        std::thread::sleep(Duration::from_millis(config::PROCESS_KILL_DELAY_MS));

        debug!(
            "Executing force termination: {} {:?}",
            self.force_cmd, self.force_args
        );
        let _ = Command::new(&self.force_cmd)
            .args(&self.force_args)
            .output();
    }
    ...
    fn for_pid(pid: u32) -> Self {
        Self {
            ...
            graceful_args: vec![
                commands::FORCE_KILL_ARGS[0].to_string(),
                commands::FORCE_KILL_ARGS[1].to_string(),
                pid.to_string(),
            ],
...
fn kill_process_tree_platform(pid: u32) {
    let _ = Command::new(commands::KILL_TREE)
        .args(&[
            commands::KILL_TREE_ARGS[0],
            commands::KILL_TREE_ARGS[1],
            commands::KILL_TREE_ARGS[2],
            &pid.to_string(),
        ])
        .output();
}
...
fn kill_process_on_port_platform(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = Command::new(commands::FUSER)
        .args(&[
            commands::FUSER_TERM_ARGS[0],
            commands::FUSER_TERM_ARGS[1],
            &port_spec,
        ])
        .output();
                    ...
                    if let Some(pid) = line.split_whitespace().last() {
                        if let Ok(pid_num) = pid.parse::<u32>() {
                            ...
                            let _ = Command::new(commands::FORCE_KILL)
                                .args(&[
                                    commands::FORCE_KILL_ARGS[0],
                                    commands::FORCE_KILL_ARGS[1],
                                    &pid_num.to_string(),
                                ])
                                .output();
                        }
