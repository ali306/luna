...
describe('WebSocketManager', () => {
  ...
  describe('WebSocket event handlers', () => {
    ...
    it('should handle onopen event correctly', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('WebSocket opened, waiting for backend pong...');
      expect(mockWebSocket.send).toHaveBeenCalledWith(
        expect.stringContaining('"type":"ping"')
      );

      // Connection callback should not be called yet (waiting for pong)
  ...
  describe('isConnected', () => {
    it('should return true when WebSocket is open and backend has responded', () => {
  ...
  describe('connection state callback', () => {
    it('should register and call connection state callback', () => {
      ...
      // Simulate backend pong response to complete connection
      if (mockWebSocket.onmessage) {
        mockWebSocket.onmessage({
          data: JSON.stringify({ type: 'pong' })
        } as MessageEvent);
    ...
    it('should call connection state callback on disconnect', () => {
      ...
      // First establish connection with pong
      if (mockWebSocket.onopen) {
        mockWebSocket.onopen(new Event('open'));
      }
      if (mockWebSocket.onmessage) {
      ...
      callback.mockClear(); // Clear the connection callback
  ...
  describe('edge cases and error handling', () => {
    it('should handle message with null data', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Received unknown WebSocket message type:', 'object');
